...
function App() {
  ...
  const getRandomColor = () => {
    ...
    return {
      bgColor: `hsl(${hue}, 80%, 90%)`,
      textColor: `hsl(${hue}, 60%, 30%)`,
      buttonColor: `hsl(${hue}, 80%, 45%)`
    };
  };
  ...
  return (
    <div className="app" style={{ backgroundColor: colors.bgColor }}>
      <div className="quote-box">
        <h1 style={{ color: colors.textColor }}>"{currentQuote.quote}"</h1>
        <p className="author" style={{ color: colors.textColor }}>- {currentQuote.author}</p>
        <button 
